...
pub fn reuse_media_part(source: &Path, destination: &Path) -> Result<()> {
    if cached_media_part(destination).is_some() {
        return Ok(());
    }
    let Some(bytes) = cached_media_part(source) else {
        return Ok(());
    };
...
fn legacy_playback_signature(detail: &VideoDetail) -> Result<String> {
    let rows = detail
        ...
        .map(|s| {
            Ok(format!(
                "{}|{}|{}",
                detail.base_url.join(&s.uri)?.path(),
                f64::from(s.duration),
                s.discontinuity
            ))
        })
...
impl CourseApi {
    ...
    pub async fn playback_media(&self, video: &VideoInfo) -> Result<PlaybackMedia> {
        ...
        Ok(PlaybackMedia {
            parts,
            cache_signature: resume_signature(&detail.base_url, &detail.playlist)?,
    ...
    pub(super) async fn bounded_media_bytes(
        &self,
        url: &url::Url,
        limit: usize,
    ) -> Result<Vec<u8>> {
        ...
        R: Fn(&str, &str, usize, &Path) -> Result<()>,
    {
        ...
        reuse(
            &signature,
            &legacy_playback_signature(&detail)?,
            detail.playlist.segments.len(),
            &resume,
        )?;
        #[cfg(unix)]
        {
        ...
        let mut bytes = 0u64;
        let mut available: Vec<u64> = (0..detail.playlist.segments.len())
            .map(|i| {
                cached_media_part(&resume.join(format!("{i:05}.ts"))).map_or(0, |b| b.len() as u64)
            })
            .collect();
        progress(MediaProgress {
            phase: "downloading",
...
mod tests {
    ...
    async fn playback_and_export_share_one_inflight_part_and_resume_after_cancellation() {
        ...
        assert_eq!(
            resume_part(&path, || async {
                panic!("completed part must remain reusable")
            })
            .await
            .unwrap(),
            bytes
        ...
        assert_eq!(
            tokio::time::timeout(
                ...
                resume_part(&second, || async { Ok(bytes.clone()) })
            )
            .await
            .unwrap()
            .unwrap(),
