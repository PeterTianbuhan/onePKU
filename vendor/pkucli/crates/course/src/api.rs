...
mod feedback;
mod learning;
pub use feedback::{AssignmentFeedback, FeedbackAttempt};
mod media;
mod recordings;
...
fn parse_submission_snapshot(body: &str) -> Result<SubmissionSnapshot> {
    ...
    let recognized = label.is_some()
        || dom
            .select(
                &Selector::parse("#uploadAssignmentFormId, #pageTitleText, span.title").unwrap(),
            )
            .next()
            .is_some();
    anyhow::ensure!(recognized, "无法识别提交记录页面");
    let files = dom
        .select(&Selector::parse("#currentAttempt_submissionList a.attachment[href]").unwrap())
        .filter_map(|a| {
            let name = a.text().collect::<String>().trim().to_string();
            let url = a.value().attr("href")?.to_string();
            (!name.is_empty()).then_some(Attachment { name, url })
        })
        .collect();
    Ok(SubmissionSnapshot { label, files })
}
...
fn parse_content_items(dom: &Html) -> Result<Vec<ContentItem>> {
    ...
    for li in dom.select(&item_sel) {
        ...
        let id = li
            .value()
            .attr("id")
            .map(|s| s.strip_prefix("contentListItem:").unwrap_or(s).to_string())
            .or_else(|| {
        ...
        let mut attachments: Vec<Attachment> = li
            .select(&attach_sel)
            .filter_map(|a| {
                let name = a.text().collect::<String>().trim().to_string();
                // 去掉 nbsp 前缀
        ...
        if item_type == ContentType::Document {
            if let Some(file_url) = url.as_ref().filter(|u| u.contains("/bbcswebdav/")) {
                if !attachments.iter().any(|a| &a.url == file_url) {
...
impl CourseApi {
    ...
    /// Desktop: the original 24h timestamp is synthetic; the server validates cookies.
    pub fn from_session_persistent() -> Result<Self> {
        let store = Store::new(APP_NAME)?;
    ...
    /// Each request owns its HTTP pool while sharing renewed cookies.
    pub fn fresh_client(&self) -> Result<Self> {
        Ok(Self {
    ...
    pub fn persist_cookies(&self) -> Result<()> {
        ...
        if store
            .load_session()?
            .is_some_and(|s| s.token == self.session_token)
        {
            store.save_cookie_store(&self.cookie_store)?;
        }
        Ok(())
    }
    ...
    pub async fn account_identity(&self) -> Result<CourseIdentity> {
        ...
        if response.url().host_str() != Some("course.pku.edu.cn") {
            return Err(anyhow!("无法确认教学网账号，请重新登录"));
        }
    ...
    async fn get_homepage(&self) -> Result<Html> {
        ...
        if resp.url().host_str() == Some("iaaa.pku.edu.cn") || resp.url().path().contains("login") {
    ...
    pub async fn list_assignments_for_course(
        ...
        for item in &assignments {
            ...
            let (deadline_raw, deadline) = match self.get_assignment(&course.id, &item.id).await {
                Ok(detail) => {
            ...
            let attempt = match self.get_assignment_attempt(&course.id, &item.id).await {
                Ok(a) => a,
                Err(_) => {
                    detail_error = true;
                    None
    ...
    pub async fn submit_assignment(
        ...
        anyhow::ensure!(
            map.get("course_id").map(String::as_str) == Some(course_id)
                && map.get("content_id").map(String::as_str) == Some(content_id),
            "提交表单目标不匹配"
        );
        ...
        let upload_client = pkuinfo_common::tls::apply_extra_roots(
        ...
        // Blackboard may redirect to its receipt. Success is reconciled by the desktop
        // against a fresh read of the submitted file; HTTP alone is not a receipt.
        anyhow::ensure!(
    ...
    /// Read existing attempt metadata without creating an attempt.
    pub async fn submission_snapshot(
        &self,
        course: &str,
        content: &str,
        ...
        let resp = self
            .client
            .get(format!("{COURSE_BASE}/webapps/assignment/uploadAssignment"))
            .query(&[
                ("mode", "view"),
                ("course_id", course),
                ("content_id", content),
            ...
            .send()
            .await?;
        anyhow::ensure!(resp.status().is_success(), "读取提交记录失败");
        if resp.url().host_str() != Some("course.pku.edu.cn") || resp.url().path().contains("login")
        {
        ...
        parse_submission_snapshot(&body)
    }
    /// Download only a file exposed by an existing assignment receipt.
    pub async fn submitted_file_bytes(&self, raw: &str, course: &str) -> Result<Vec<u8>> {
        let base = url::Url::parse(COURSE_BASE)?;
        let u = base.join(raw)?;
        anyhow::ensure!(
            u.scheme() == "https"
        ...
        let mut resp = self.client.get(u).send().await?.error_for_status()?;
        anyhow::ensure!(
            resp.url().host_str() == Some("course.pku.edu.cn")
        ...
        while let Some(chunk) = resp.chunk().await? {
            anyhow::ensure!(
                bytes.len() + chunk.len() <= 25 * 1024 * 1024,
                "提交附件超出核对上限"
            );
    ...
    pub async fn list_announcements(&self, course_id: &str) -> Result<Vec<Announcement>> {
        ...
        for li in dom.select(&list_sel) {
            ...
            let id = li.value().attr("id").unwrap_or("").to_string();
            let mut original = url::Url::parse(&format!(
                "{COURSE_BASE}/webapps/blackboard/execute/announcement"
            ))?;
            original.query_pairs_mut().extend_pairs([
                ("method", "search"),
...
mod onepku_tests {
    ...
    fn document_title_is_downloadable_and_assignment_id_is_normalized() {
        ...
        let rows = parse_content_items(&doc).unwrap();
        assert_eq!(rows[0].id, "_7_1");
        assert_eq!(rows[0].item_type, ContentType::Assignment);
        assert_eq!(rows[1].attachments.len(), 1);
        assert_eq!(rows[1].attachments[0].name, "lecture.pdf");
    }
}

#[cfg(test)]
mod desktop_submission_tests {
    use super::*;
    #[test]
    fn reading_a_form_is_not_a_submission_receipt() {
        let v = parse_submission_snapshot(
            "<form id='uploadAssignmentFormId'><input name='nonce' value='private'></form>",
        )
        .unwrap();
        assert!(v.label.is_none());
        assert!(v.files.is_empty());
        assert!(parse_submission_snapshot("<html>Unavailable</html>").is_err());
    }
    #[test]
    fn receipt_has_only_current_attempt_files() {
        let v=parse_submission_snapshot("<h3 id='currentAttempt_label'>尝试 1</h3><ul id='currentAttempt_submissionList'><li><a class='attachment' href='/webapps/assignment/download?course_id=_1_1&amp;file_id=2'>answer.txt</a></li></ul><a class='attachment' href='/other'>teacher.pdf</a>").unwrap();
        assert_eq!(v.label.as_deref(), Some("尝试 1"));
        assert_eq!(v.files.len(), 1);
        assert_eq!(v.files[0].name, "answer.txt");
    }
}
