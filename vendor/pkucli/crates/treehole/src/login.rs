...
pub fn iaaa_config(device_uuid: &str) -> IaaaConfig {
    IaaaConfig {
...
/// 获取或生成设备 UUID
pub fn get_device_uuid(store: &Store) -> String {
    // 尝试从已有 session 中读取
...
pub async fn complete_treehole_login(
    store: &Store,
    iaaa_token: &str,
    device_uuid: &str,
) -> Result<()> {
...
pub async fn complete_gui_login(store: &Store, iaaa_token: &str, device_uuid: &str) -> Result<()> {
    ...
    // Verification is surfaced in the GUI.
