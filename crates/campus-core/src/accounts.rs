...
pub(crate) fn root() -> Result<PathBuf> {
    Ok(directories::ProjectDirs::from("me", "petertian", "OnePKU")
        .ok_or_else(|| anyhow!("无法定位账号目录"))?.data_local_dir().to_owned())
}
pub(crate) fn account_key(id: &str) -> String {
    format!("{:x}", Sha256::digest(format!("course.pku.edu.cn/blackboard-user/{id}")))
}
...
fn read_json<T: serde::de::DeserializeOwned>(path: &Path) -> Option<T> {
    ...
    if file.metadata().ok()?.len() > 16384 { return None; }
    let mut bytes = vec![];
...
fn write_json(path: &Path, value: &impl Serialize) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        let mut file = fs::OpenOptions::new().write(true).create_new(true).private_mode().open(&temp)?;
...
pub(crate) fn binding(root: &Path, generation: &str) -> Option<String> {
    ["course-sessions", "subtitle-accounts"].iter().find_map(|folder| {
        read_json::<String>(&root.join(folder).join(format!("{generation}.json"))).filter(|v| digest(v))
    })
}
fn save_verified(root: &Path, generation: &str, identity: &pku_course::api::CourseIdentity) -> Result<String> {
    let account = account_key(&identity.id);
    if binding(root, generation).is_some_and(|old| old != account) {
        bail!("教学网账号身份发生变化，请重新登录后重试");
    }
    write_json(&root.join("course-accounts").join(format!("{account}.json")), &json!({
    ...
    write_json(&root.join("course-sessions").join(format!("{generation}.json")), &account)?;
    Ok(account)
}
pub(crate) fn bound_generations(root: &Path, account: &str) -> Vec<String> {
    ...
    for folder in ["course-sessions", "subtitle-accounts"] {
        ...
        if fs::symlink_metadata(&dir).is_ok_and(|m| platform::is_link(&m)) { continue; }
        let Ok(entries) = fs::read_dir(dir) else { continue; };
        for entry in entries.flatten() {
            let name = entry.path().file_stem().and_then(|s|s.to_str()).unwrap_or("").to_owned();
            if digest(&name) && binding(root, &name).as_deref() == Some(account) { out.insert(name); }
        }
    }
...
impl Core {
    pub(crate) async fn course_account(&self, generation: &str) -> Result<String> {
        ...
        if let Some(account) = &saved {
            let metadata: Option<Value> = read_json(&root.join("course-accounts").join(format!("{account}.json")));
            if metadata.is_some_and(|v| v["version"] == 1 && v["account"] == *account
                && v["provider"] == "course.pku.edu.cn"
                && v["providerUserId"].as_str().is_some_and(|id| !id.is_empty() && account_key(id) == *account)) {
                if generation != fingerprint("course") { bail!("账号已更新，请重试"); }
...
mod tests {
    ...
    fn login_rotation_groups_only_verified_same_accounts() {
        ...
        let identity = pku_course::api::CourseIdentity { id:"_10_1".into(), user_name:"20260001".into() };
        let first="a".repeat(64); let second="b".repeat(64); let third="c".repeat(64);
        let account=save_verified(dir.path(),&first,&identity).unwrap();
        assert_eq!(save_verified(dir.path(),&second,&identity).unwrap(),account);
        let other=pku_course::api::CourseIdentity { id:"_11_1".into(), user_name:"20260002".into() };
        assert_ne!(save_verified(dir.path(),&third,&other).unwrap(),account);
        assert_eq!(bound_generations(dir.path(),&account),vec![first.clone(),second]);
